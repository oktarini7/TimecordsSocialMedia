...
$statuslist = "";
if($isOwner == "yes"){
	$status_ui = '<textarea id="statustext" onkeyup="statusMax(this,250)" placeholder="What are you thinking about '.$u.'?"></textarea>';
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
    ...
    if($replynumrows > 0){
        while ($row2 = mysqli_fetch_array($query_replies, MYSQLI_ASSOC)) {
			...
			$status_replies .= '<div id="reply_'.$statusreplyid.'" class="reply_boxes"><b>Reply by <a href="user.php?u='.$replyauthor.'">'.$replyauthor.'</a> '.$replypostdate.':</b> '.$replyDeleteButton.'<br />'.$replydata.'</div>';
        }
    }
